...
func main() {
	...
	db, err := openDB(*dsn) if err != nil {
		errorLog.Fatal(err) }
	defer db.Close()
	templateCache, err := newTemplateCache() if err != nil {
		errorLog.Fatal(err) }
...
errorLog: infoLog: snippets: templateCache: formDecoder:
	errorLog,
		infoLog, &models.SnippetModel{DB: db}, templateCache,
		formDecoder,
